...
export default {
  ...
  mounted() {
    ...
    this.minTransLateY = -this.bgImageHeight + STATE_HEIGHT
  ...
  watch: {
    scrollY(newY) {
      ...
      if (newY > 0) {
        zIndex = 10
        scaleValue = percent + 1
        this.$refs.bgImage.style[`transform`] = `scale(${scaleValue})`
      } else {
        blur = Math.min(20 * percent, 20)
        this.$refs.bgLayer.style['backdrop-filter'] = `blur(${blur}px)`
        this.$refs.bgLayer.style['webkitBackdrop-filter'] = `blur(${blur}px)`
      }
